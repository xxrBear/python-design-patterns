...
**定义**：代理模式为某个对象提供一个代理对象，并由代理对象控制对原对象的访问
...
| 类型         | 描述                                                     |
| ------------ | -------------------------------------------------------- |
| 远程代理     | 为在不同地址空间的对象提供访问接口（如 RPC、数据库连接） |
| 虚拟代理     | 延迟加载资源，只有在需要时才创建真实对象（如大图加载）   |
| 保护代理     | 控制权限，比如不同用户是否可以访问目标对象               |
| 智能引用代理 | 在访问对象时自动执行某些附加操作（如引用计数、日志记录） |
