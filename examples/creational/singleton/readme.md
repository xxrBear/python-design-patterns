...
## 简介
单例模式是最常见、最简单的设计模式之一，其核心思想是：保证一个类在系统中只能有一个实例，并提供全局访问点。这在需要统一管理某些全局资源或状态时非常有用
...
这是最简单的单例模式，如果你创建这个类的实例，你会发现无论创建多少次，实例对象的内存地址都是相同的
...
Python 的模块导入有且只会有一次，如果你不显示的使用`reload`方法调用的话，无论`import`多少次，模块只会加载一次，利用这个特性，我们只需要在模块中实例话这个类，然后在使用它的地方导入这个类实例即可实现单例模式
